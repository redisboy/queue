package com.suntendy.queue.employee.util.servlet;
...
import com.suntendy.queue.employee.util.teetaa.util.*;
...
public class ZoomImage extends HttpServlet
{
			...
			throws ServletException, IOException
	{
		...
		System.out.println("url :"+request.getRealPath("")+"/upload/"+picture);
		Rectangle rec = new Rectangle(cutLeft,cutTop,dropWidth,dropHeight);
		File file = new File(request.getRealPath("")+"/upload/"+picture);
		//BufferedImage image = ImageIO.read(new File(request.getRealPath("")+"/UploadPhoto/"+picture));  
		saveSubImage(new File(request.getRealPath("")+"/upload/"+picture),file,imageWidth,imageHeight,rec);
		response.sendRedirect("/ZoomImageDemo/uploadimage.jsp?Picurl="+picture+"&step=3");
	}
