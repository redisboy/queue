...
public final class UUID implements Serializable, Comparable {
	...
	public static UUID fromString(String s) {
		String as[] = s.split("-");
		if (as.length != 5)
			throw new IllegalArgumentException((new StrBuilder()).append(
					"Invalid UUID string: ").append(s).toString());
		for (int i = 0; i < 5; i++)
			as[i] = (new StrBuilder()).append("0x").append(as[i]).toString();
	...
	public String toString() {
		return (new StrBuilder()).append(digits(mostSigBits >> 32, 8))
