...
<%
	List<QueueWaitCount> list = (List<QueueWaitCount>) request.getAttribute("list");
	...
	%>
		window.onload=function(){
			...
			<%}%>
			var strXML = "<chart xAxisName='' yAxisName='' caption='队列排队统计' showToolTip='0' shownames='1' showvalues='1' showSum='1' useRoundEdges='1'>";
...
function query() { 
	...
	document.form1.action = "dlpdtj2.action";
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">队列排队统计</span>
    </td>
    ...
    <form action="" method="post" name="form1">
     ...
     <input name="deptHall" id="deptHall" type="hidden" value="<%=deptHall %>"/>
          ...
          <select name="tjms" id="tjms" >
	        ...
	        <td class=tableheader1 align=middle width=10% height=30>窗口号:</td>
	        <td align=middle width=150 bgColor=#f1f9fd height=30>&nbsp;
	          <select name="barid" id="barid" >
                        <option value="0">选择窗口号</option>
                        <c:forEach items="${barLsit}" var="list">
                            <option value="${list.windowid}">${list.windowid}</option>
                        </c:forEach>
                </select>
	        </td>
            </tr>
	    			...
	    			<c:forEach items="${list}" var="item" varStatus="status">
	    				<c:if test="${(status.index%2)!=0}">
	    					<tr  style="text-align:center">
		    					...
		    					<td class="even" style="text-align:center"  width="10%" onmouseover="moveInTdLight(this)" onmouseout="moveOutTdUnLight(this)">${item.total}</td>
		    			...
		    			<c:if test="${(status.index%2)==0}">
	    					<tr  style="text-align:center">
		    					...
		    					<td class="odd" style="text-align:center"  width="10%" onmouseover="moveInTdLight(this)" onmouseout="moveOutTdUnLight(this)">${item.total}</td>
				...
				<td> 
					<ec:table items="list" var="list" 
							...
							<ec:row style="text-align:center">
								...
								<ec:column width="10%" property="ave_worktime" title="平均办理时间" style="text-align:center" editable="false" />
							</ec:row>
							<c:if test="${msg != null}">
								<ec:extendrow>
