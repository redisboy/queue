...
<%  
    ...
    response.setHeader("Content-Disposition", "attachment;filename=" + new String("照片信息查询导出结果.xls".getBytes("gb2312"), "iso8859-1"));  
   ...
   // ws.mergeCells(0, 0, 3, 0);//合并单元格  
	...
	ws.addCell(new Label(1, 1, "证件号码"));  
	ws.addCell(new Label(2, 1, "取号时间"));
	ws.addCell(new Label(3, 1, "叫号时间"));
	ws.addCell(new Label(4, 1, "身份证照片"));
	ws.addCell(new Label(5, 1, "取号照片"));
	ws.addCell(new Label(6, 1, "业务办理照片"));
    for(int i=0;i<list.size();i++) {   
	    ws.addCell(new Label(0, i+3, list.get(i).getSerialNum()));
	    ws.addCell(new Label(1, i+3, list.get(i).getIDNumber()));
	    ws.addCell(new Label(2, i+3, list.get(i).getEnterTime()));
	    ws.addCell(new Label(3, i+3, list.get(i).getBeginTime()));
	    ws.addCell(new Label(4, i+3, list.get(i).getSfzphotoStr()));
	    ws.addCell(new Label(5, i+3, list.get(i).getQuhaoPhotoStr()));
	    ws.addCell(new Label(6, i+3, list.get(i).getPhotoBase64Str()));
	  }
