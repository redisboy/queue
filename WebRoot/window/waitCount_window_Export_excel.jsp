...
<%  
	List<WindowCount> list = (List<WindowCount>)request.getSession().getAttribute("list");
    response.reset();  
    response.setContentType("application/vnd.ms-excel;charset=UTF-8");  
    response.setHeader("Content-Disposition", "attachment;filename=" + new String("窗口排队统计.xls".getBytes("gb2312"), "iso8859-1"));  
  
    OutputStream os = response.getOutputStream();  
  
    WritableWorkbook wwb = null;  
    wwb = Workbook.createWorkbook(os);//将 WritableWorkbook 写入到输出流  
    WritableSheet ws = wwb.createSheet("窗口排队统计", 0);//创建第一个sheet  
    ...
    if(role.equals("0")){
	    ...
	    for(int i=0;i<list.size();i++) {   
		    ...
		    ws.addCell(new Label(4, i+1, list.get(i).getPersonCount(),headerFormat));
		...
		for(int i=0;i<list.size();i++) {
		    ...
		    ws.addCell(new Label(3, i+1, list.get(i).getPersonCount(),headerFormat));
		...
		for(int i=0;i<list.size();i++) {
		    ...
		    ws.addCell(new Label(2, i+1, list.get(i).getPersonCount(),headerFormat));
