...
<%
	List<WindowCount> list = (List<WindowCount>) request.getAttribute("list");
	...
	%>
		window.onload=function(){
			...
			<% for (WindowCount qwc : list) {%>
			str1 += "<category label='<%=qwc.getBarid()%>号窗口' />";
			str2 += "<set value='<%=qwc.getPersonCount()%>' />";
			str3 += "<set value='<%=qwc.getAve_waittime()%>' />";
			str4 += "<set value='<%=qwc.getAve_worktime()%>' />";
			<%}%>
			var strXML = "<chart xAxisName='' yAxisName='' caption='窗口排队统计' showToolTip='0' shownames='1' showvalues='1' showSum='1' useRoundEdges='1'>";
...
function query() { 
	...
	document.form1.action = "ckpjtj.action";
	document.form1.submit();
}
function hpExport(obj){
	window.location.href="/queue/window/waitCount_window_Export_excel.jsp";
    return true;
}
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">窗口排队统计</span>
    </td>
	    			...
	    			<c:forEach items="${list}" var="item" varStatus="status">
	    				<c:if test="${(status.index%2)!=0}">
	    					<tr  style="text-align:center">
		    					...
		    					<td class="even" style="text-align:center"  width="10%" onmouseover="moveInTdLight(this)" onmouseout="moveOutTdUnLight(this)">${item.personCount}</td>
		    			...
		    			<c:if test="${(status.index%2)==0}">
	    					<tr  style="text-align:center">
		    					...
		    					<td class="odd" style="text-align:center"  width="10%" onmouseover="moveInTdLight(this)" onmouseout="moveOutTdUnLight(this)">${item.personCount}</td>
    	...
    	<table width="98%" align="center" border="0" cellpadding="0" cellspacing="0" >
			...
			<ec:table items="list" var="list" 
					...
					<ec:row style="text-align:center">
						<ec:column width="10%" property="barid" title="窗口名称" style="text-align:center" editable="false" />
						<ec:column width="10%" property="xm" title="经办人" style="text-align:center" editable="false" />
						<ec:column width="10%" property="personCount" title="人数" style="text-align:center" editable="false" />
						<ec:column width="10%" property="ave_waittime" title="平均等待时间(分)" style="text-align:center" editable="false" />
	                    <ec:column width="10%" property="ave_worktime" title="平均办理时间(分)" style="text-align:center" editable="false" />
					</ec:row> 
					<c:if test="${msg != null}">
						<ec:extendrow>
