...
function showCYMessage(data) {
	//查验综合屏
	var ou1 = $('#date_u1');
	var newLi1 = "<li style='text-align:left;width:100%;list-style:none;border-bottom: none;line-height:28px;height:28px;font-size: 28px;'>"
			+"请"+ data.SerialNum + "号入场查验-车牌号:"+data.carNumber+"</li>";
		<!--	+"-----------------"+ data.SerialNum + "号车主请入场查验-----------------</li>";-->
	if (5 <= ou1.find("li").length) {
		ou1.find("li:first").remove();
	}
	ou1.append(newLi1);
	//查验小车屏
	if(data.carType=="0"){
		if(data.sendstatus0 % 2 == 0){
			var ou1 = $('#date_u2');
			var newLi1 = "<li style='width:100%;list-style:none;border-bottom: none;line-height:40px;height:40px;font-size: 40px;'>"
					+ data.SerialNum + "</li>";
			if (3 <= ou1.find("li").length) {
				ou1.find("li:first").remove();
			}
			ou1.append(newLi1);
		}else{
