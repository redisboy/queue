...
function showMessage(data) {
    ...
    var newLi = "<li id='" + liId + "' style='font-size:28px;width:100%;text-align:center'><font color='red'>"+data.serialNum+"</font>号请到<font color='red'>"+data.winNum+"</font>窗口</li>";
	...
	$.ajax({
        ...
        success: function(data) {
			...
			for(var i=0;i<ywAndddrs.length;i++){
				...
				$("#ywmc").append("<li style='font-size:25px;width:100%;height:20px;' id='"+ywAndddrs[i].id+"name'>"+ywAndddrs[i].name+"</li></br>");
				$("#ywmc").append("<li style='font-size:25px;width:100%;height:20px'/>");
				$("#ddrs").append("<li style='font-size:25px;width:100%;height:20px' id='"+ywAndddrs[i].id+"ddrs'>"+ywAndddrs[i].ddrs+"</li></br>");
				$("#ddrs").append("<div style='font-size:25px;width:100%;height:20px'/>");
				$("#nextNum").append("<li style='font-size:25px;width:100%;height:20px'  id='"+ywAndddrs[i].id+"nextNum'>"+"</li></br>");
				$("#nextNum").append("<li style='font-size:25px;width:100%;height:20px'/>");
			}
			//遍历窗口号及业务量到页面
