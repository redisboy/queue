...
function showMessage(data) {
    ...
    var newLi = "<li id='" + liId + "' style='font-size:20px;width:100%;text-align:center'><font color='red'>"+data.serialNum+"</font>号到<font color='red'>"+data.winNum+"</font>窗</li>";
    ...
    var newLi2 = "<li id='" + liId +"2"+ "' style='font-size:20px;width:100%;text-align:center'><font color='red'>"+data.serialNum+"</font>号到<font color='red'>"+data.winNum+"</font>窗</li>";
	...
	$.ajax({
        ...
        success: function(data) {
			...
			for(var i=0;i<ywAndddrs.length;i++){
				...
				$("#ywmc").append("<li style='font-size:13px;width:100%;height:20px;' id='"+ywAndddrs[i].id+"name'>"+ywAndddrs[i].name+"</li>");
				$("#ddrs").append("<li style='font-size:15px;width:100%;height:20px' id='"+ywAndddrs[i].id+"ddrs'>"+ywAndddrs[i].ddrs+"</li>");
				$("#nextNum").append("<li style='font-size:15px;width:100%;height:20px'  id='"+ywAndddrs[i].id+"nextNum'></li>");
			}
			//遍历窗口号及业务量到页面
			...
			for(var i=0;i<screenAndywl.length;i++){
				...
				$("#windowNum").append("<li style='font-size:15px;width:100%;height:20px' id='"+screenAndywl[i].windowNum+"winNum'>"+screenAndywl[i].windowNum+"</li>");
				$("#ywl").append("<li style='font-size:15px;width:100%;height:20px' id='"+screenAndywl[i].windowNum+"ywl'>"+screenAndywl[i].ywl+"</li>");
				$("#hm").append("<li style='font-size:15px;width:100%;height:20px' id='"+screenAndywl[i].windowNum+"hm'></li>");
...
function showLzxx(data) {
	...
	if(xm.length>6){
		...
		var newLi2 = "<li id='" + liId2 + "' style='width:100%;list-style:none;border-bottom: none;line-height:23px;height:23px;font-size: 23px;'>"+xm+"请到"+data.barid+"号窗口</li>";
