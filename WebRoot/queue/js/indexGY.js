...
GenerateNumber = (function() {
    ...
    var mainType = "<li><a href='javascript:void(0);' class='btn_ywlx' onfocus='this.blur()' onclick='gn.showChildType(\"02\")'>驾驶人<a></li>";
		...
		$.each(bsnsTypes, function(key, obj) {
			...
			if(!isEmpty(obj.twotype) && obj.twotype == flag){
				...
				var names = obj.name.split("@");
					//isOpenTztd 是否开启通知提档 0开启 1关闭
					if("0"==obj.isOpenTztd){
         				newLi = "<li><a href='javascript:void(0);' class='btn_ywlx' onmouseout='out()' onmouseover='over(\""+helps+"\",\""+dispy+"\")' onfocus='this.blur()' onclick='gn.checkCl(\""+key+"\",\""+obj.id+"\",\""+flag+"\",\""+obj.isOpenZhiWen+"\")'>"+names[0]+" "+names[1]+"<a></li>";
	         		}else{
	         			newLi = "<li><a href='javascript:void(0);' class='btn_ywlx' onmouseout='out()' onmouseover='over(\""+helps+"\",\""+dispy+"\")' onfocus='this.blur()' onclick='gn.saveZhiWen(\""+key+"\",\""+obj.id+"\",\""+flag+"\",\""+obj.isOpenZhiWen+"\")'>"+names[0]+" "+names[1]+"<a></li>";
	         		}
	         		if(4 >= index){content1 += newLi;} 
	         		if(10 >= index && 5 <= index){content2 += newLi;}
         			index ++;
...
//			obj.value = obj.value.replace(/[^A-Za-z0-9_]/g, "");
		...
		this.checkCl =function(key,id,flag,isOpenZhiWen){
			var result = pdqhl(id);
			if(result[0]){
				zxcl(key,id,flag,isOpenZhiWen,isOpenTztd);
			}else{
				//alert(result[1]);
