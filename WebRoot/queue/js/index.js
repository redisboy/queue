...
GenerateNumber = (function() {
	...
	function showTwoType(flag){
		...
		var chepainum = window.prompt("请输入车牌号码", "");
		objDiv.html(jdctype);
        objDiv.css("padding-left", "70px");
		 $.ajax({type: "POST", cache: false, async: false, dataType: "json",
		        url: 'number/checkStatus.action?flag='+flag+"&chepainum="+chepainum,
...
//					     });
						...
						if(agentfalg == 1){
							...
							if(isAgen == 1){//0非代理人 1代理人 
								var printInfo=data.str.replace("@",data.queueName).replace("*",data.serialNum).replace("#",data.peoNum).replace("+","单位："+Unit).replace("!",count+"笔").replace("$",vDate+" "+vTime).replace("^",IDNumber).replace("%","<font size=3>姓名：</font>"+NameA+"").replace("?",waitingarea).replace("~",tb);
							}else{
...
//			$("#index").css("display", "none");
		...
		this.bcTztd = function (IDNumber,key,id,flag,isOpenZhiWen){
