...
GenerateNumber = (function() {
    ...
    var keys = "",ids = "",flags="",isZhiWens="",istztds="";
    var jdcnum="",jdctypes="";//机动车
    ...
    var mainType = "<li><a href='javascript:void(0);' class='btn_ywlx' onfocus='this.blur()' onclick='gn.checked(\"01\")'>机动车<a></li>"
    	...
    	+ "<li><a href='javascript:void(0);' class='btn_ywlx' onfocus='this.blur()' onclick='gn.checked(\"04\")'>违法<a></li>";
    ...
    var jdctype = "<div id='jdctype' style='z-index:-1;position:absolute;left:110px;top:200px;'><table width='100%' height='200' border='1' style='background-color:#7AB4E2' cellpadding='0' cellspacing='0'><tr><td>号牌种类:</td><td><div id='hpzl'></div></td><td>号牌号码:</td><td><div id='hphm'></div></td></tr>"
    				...
    				+"</table><input type='button' class='button' value='下一步' onclick='gn.jctztdCheck();'/></div>";
    ...
    var jdcxinxi = "<div class='inner_fsfz' id='jdctype'><p style='color: red;font-size: 30px;'>请输入机动车信息</p>"
			...
			if(cywffs=='0'){//人工查验
				...
				this.jdcbtnOk = function(){
			        ...
			        objDiv.css("padding-left", "70px");
					 $.ajax({type: "POST", cache: false, async: false, dataType: "json",
					        url: 'number/checkStatus.action?flag='+flags+"&chepainum="+jdcnum+"&jdctypes="+jdctypes,
				...
				this.jdcbtnCancel = function(){
		        	...
		        	objDiv.css("padding-left", "70px");
				 	$.ajax({type: "POST", cache: false, async: false, dataType: "json",
				        url: 'number/checkStatus.action?flag='+flags+"&IDNumber="+IDNumber,
				        success: function(data) {
					 	if(data.success){
				                $("#xm").html(data.xm);
				                $("#zjcx").html(data.zjcx);
				...
				this.jdcbtnOk = function(){
					...
					$("#jdcnum").val().replace(/[^A-Za-z0-9_]/g, "");
					 $.ajax({type: "POST", cache: false, async: false, dataType: "json",
					        url: 'number/checkStatusZD.action?flag='+flags+"&chepainum="+jdcnum+"&jdctypes="+jdctypes+"&id="+ids,
					        success: function(data) {
							 	if(data.success){
//							 		check(flag);
//							 		gn.createNumber();
...
//						gn.createNumber();
				 	...
				 	$.ajax({type: "POST", cache: false, async: false, dataType: "json",
				        url: 'number/checkStatusZD.action?flag='+flags+"&IDNumber="+IDNumber+"&id="+ids,
				        success: function(data) {
					 		if(data.success){
//						 		check(flag);
//					 			gn.createNumber();
...
//					gn.createNumber();
		...
		}else if(istztds=='0' && isOpenTztd=='0'){
			var objDiv = $("#tdiv").empty();
				objDiv.html(jdcxinxi);
				this.jdcbtnOk = function(){
					jdctypes = $("#jdcsel").val();
		...
		$.each(bsnsTypes, function(key, obj) {
			if(!isEmpty(obj.twotype) && obj.twotype == flag){
				var names = obj.name.split("@");
					//isOpenTztd 是否开启通知提档 0开启 1关闭
//					if("0"==obj.isOpenTztd){
         				newLi = "<li><a href='javascript:void(0);' class='btn_ywlx' onfocus='this.blur()' onclick='gn.checkCl(\""+key+"\",\""+obj.id+"\",\""+flag+"\",\""+obj.isOpenZhiWen+"\",\""+obj.isOpenTztd+"\")'>"+names[0]+" "+names[1]+"<a></li>";
//	         		}else{
//	         			newLi = "<li><a href='javascript:void(0);' class='btn_ywlx' onfocus='this.blur()' onclick='gn.saveZhiWen(\""+key+"\",\""+obj.id+"\",\""+flag+"\",\""+obj.isOpenZhiWen+"\")'>"+names[0]+" "+names[1]+"<a></li>";
//	         		}
	         		if(4 >= index){content1 += newLi;} 
...
//		  	showBackBtn();
		...
		this.createNumber = function() {;
		    ...
		    if (!isEmpty(agentInfo) && agentInfo.isAgnet) {
			    ...
			    //判断取号次数
			    gn.numberCount(IDNumber,0,ids.substring(1),agentInfo.agentId,count);
			    personType = 2; //代理人
		    }else{
		    	gn.numberCount(IDNumber,1,ids.substring(1),"",count);//判断取号次数
		    }
		    if(isCountSet == 1){
		     	agentInfo="";
		     	//返回按钮隐藏
						...
						if(agentfalg == 1){
							...
							if(banliTypesVo==1){
								if(isAgen == 1){//0非代理人 1代理人 
									isAgen=0;
									var printInfo=data.str.replace("@",data.queueName).replace("*",data.serialNum).replace("#",data.peoNum).replace("+","单位："+Unit).replace("!",count+"笔").replace("$",vDate+" "+vTime).replace("^",IDNumber).replace("%","<font size=3>姓名：</font>"+nameB+"").replace("?",waitingarea).replace("~",tb);
									Unit="";
								}else{
									var printInfo=data.str.replace("@",data.queueName).replace("*",data.serialNum).replace("#",data.peoNum).replace("+","").replace("!",count+"笔").replace("$",vDate+" "+vTime).replace("^",IDNumber).replace("%","<font size=3>姓名：</font>"+nameB+"").replace("?",waitingarea).replace("~",tb);
								}
								banliTypesVo = 0;
							}else{
...
//			$("#index").css("display", "none");
		...
		//是否开启通知提档
		function zxcl(key,id,flag,isZhiWen,istztd){
//			$.ajax({type: "POST", cache: false,dataType: "json",async: false,
...
//			obj.value = obj.value.replace(/[^A-Za-z0-9_]/g, "");
		...
		this.checkCl =function(key,id,flag,isZhiWen,istztd){
			zxcl(key,id,flag,isZhiWen,istztd);
		}
		this.jctztdCheck = function(){
			jctztd();
		}
		this.numberCount=function(IDNumber,isAnget,typeId,agentId,count){
			numberCount(IDNumber,isAnget,typeId,agentId,count);
		}
		this.saveZhiWen=function(){
			//判断是否录入指纹
			if(0 == isZhiWens){
				//判断是否需要保存指纹
				$.ajax({type: "POST", cache: false,dataType: "json",async: false,
