...
GenerateNumber = (function() {
    ...
    var SFZnumber="",SFZNameA="",SFZSex="",SFZNation="",SFZBorn="",SFZAddress="",SFZPolice="",SFZUserLifeB="",SFZUserLifeE="",SFZBase64="",DYnumber="",DYname="",SFZBase64ForFace="";//用于表单打印传值
       ...
       //间隔读取身份证信息
         ...
         this.rzbd = function(){
			...
			if (0 == nRet) {
			    ...
			    SFZBase64ForFace = readSysCard.Base64Photo;//身份证流
				OpenVideo();
				DrawFaceRect();
				compareVideoBase64(SFZBase64ForFace);
				upIcardPic1(readSysCard.Base64Photo,number,"1");
...
//			    	gn.numberCount(IDNumber,1,id.substring(1),"",count);//判断取号次数
		    ...
		    if(!isEmpty(count) && count != 0){
		    	...
		    	$.post("number/generateNumber.action?PNumber="+phoneNumber+"&cartype="+cartype2+"&carNumber="+encodeURI(encodeURI(carnumber))+"&SFZBase64ForFace="+SFZBase64ForFace+"&"+parseInt(Math.random()*100000), params, function (data, status) {
					if (data.isSuccess) {
...
function upIcardPic1(base64,cardNumber,rzbdbj){
	var params = "base64picPhoto=" + encodeURIComponent(base64) + "&cardNumber="+cardNumber;
	$.ajax({type:"POST", cache:false, data:params,
			url:"number/uploadPic1.action",
			success:function (picres) {
//			if(0 == picres){
//				var features = "status=no,resizable=no,top=0,left=0,scrollbars=no,width=300,height=400"
//				+ "titlebar=no,menubar=no,location=no,toolbar=no,z-look=yes" 
