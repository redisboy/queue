...
//					     });
						...
						if (isEmpty(orderForOther)) {
							...
							if ("是" == orderForOther) {
								...
								nameB = reservationUserName;
								IDNumberB = "*" + idNo.substring(idNo.length-6);
						...
						if (isEmpty(NameA)) {
							printInfo = printInfo.replace("&reservationUserName", "");
						} else {
						...
						if (isEmpty(IDNumberB)) {
							...
							printInfo = printInfo.replace("&IDNumberB", "代办人证件号 : " + "*" + IDNumberB + "<br>");
						}
						
						if(isEmpty(CYLSNum)){
							printInfo = printInfo.replace("&CYLSNum", "");
						}else{
							printInfo = printInfo.replace("&CYLSNum", "查验流水号为 : " + "*" + CYLSNum + "<br>");
						}
...
//			obj.value = obj.value.replace(/[^A-Za-z0-9_]/g, "");
		...
		this.wybl = function(flag){
			  	...
			  	if (0 == nRet) {
				    ...
				    SFZBase64 = readSysCard.Base64Photo;//身份证流
			  		...
			  		if (banliTypes == 1) {
			  			...
			  			banliTypesVo = 1;
			  			banliTypes = 0;
			  			document.getElementById("msg2").innerHTML ="请&nbsp;放&nbsp;入&nbsp;身&nbsp;份&nbsp;证！";
						isclick = false;
			  			return;
			  		} else {
			...
			var yyafter = '0'; //延后时间
		  	//查询预约信息并返回 打印信息
		  	$.ajax({type: "POST", cache: false,dataType: "json",async: false,data:{"yybefore":yybefore,"yyafter":yyafter,"sfzmhm":IDNumber,"IDType":IDType,"SFZNameA":SFZNameA},
				...
				success: function (data) {
					...
					if('0'==data.code){
						...
						if("0"==data.isQd){
							...
							$("#LSH").css("display", "");
							$("#LSHNum").val("");
							document.getElementById("msg").innerHTML ="请输入查验流水号的后六位进行取号!";
						}else{
							qhao('business');
						}
					}
		...
		this.showWYGetNumber = function(data){
			...
			if((hour == 8 && minute <15) || hour < 8 || hour > 17 || (hour == 17 && minute >0)){
        		 alert("取号机取号时间为每天8:15 - 17:00");
        		 return false;
        	}
			if (data == '0') {
				//网约本人办理
