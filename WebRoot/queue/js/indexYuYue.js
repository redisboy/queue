...
GenerateNumber = (function() {
    ...
    var yyxxbj=0,yyxxmsg="";//0为预约信息无效 1为有效
	...
	function displayType(type, number,NameA,waitingarea) {
	 ...
	 //   if (isEmpty(bsnsTypes)) {
	        $.ajax({type: "POST", cache: false, async: false, dataType: "json",
		        url: 'number/bsnsType.action?yybj=1&'+parseInt(Math.random()*100000),
	  ...
	  //  }
	    if (!isSuccess) { return; }
	    gn.showChildType('01');
//	     gn.createNumber("d"+yyxxmsg.split("@")[1],yyxxmsg.split("@")[1]);
//	    if (isChange) {
		...
		$.each(bsnsTypes, function(key, obj) {
			if(!isEmpty(obj.twotype)){
				...
				var names = obj.name.split("@");
					//isOpenTztd 是否开启通知提档 0开启 1关闭
					if("0"==obj.isOpenTztd){
						if(obj.id == yyxxmsg.split("@")[1]){
							bj=1;
							gn.checkCl(key,obj.id,flag,obj.isOpenZhiWen);
						}
//         				newLi = "<li><a href='javascript:void(0);' class='btn_ywlx' onfocus='this.blur()' onclick='gn.checkCl(\""+key+"\",\""+obj.id+"\",\""+flag+"\",\""+obj.isOpenZhiWen+"\")'>"+names[0]+" "+names[1]+"<a></li>";
	         		}else{
	         			if(obj.id == yyxxmsg.split("@")[1]){
	         				bj=1;
	         				gn.saveZhiWen(key,obj.id,obj.isOpenZhiWen,flag);
	         			}
...
//		  	showBackBtn();
		...
		this.createNumber = function(id) {
		    ...
		    if(!isEmpty(count) && count != 0){
		    	alert("obj.id业务类型id="+obj.id);
		    	alert("IDType="+IDType);
		    	alert("IDNumber="+IDNumber);
		    	alert("IDNumberB="+IDNumberB);
		    	alert("count笔数="+count);
		    	alert("personType="+personType);
		    	alert("obj.prefix队列前缀="+obj.prefix);
		    	alert("obj.code队列编号="+obj.code);
		    	alert("obj.name="+obj.name);
		    	alert("obj.flag="+obj.flag);
		    	var params = {businessType:obj.id, IDType:IDType, IDNumber:IDNumber,IDNumberB:IDNumberB, businessCount:count,
					personType:personType, prefix:obj.prefix, code:obj.code, typeName:obj.name, flag:obj.flag,yybj:1};
...
//			$("#index").css("display", "none");
		...
		//验证预约信息
		function yzyyxx(IDNumber){
			$.ajax({type: "POST", cache: false,dataType: "json",async: false,
					url: 'queryReservation.action?idnumber='+IDNumber+"&"+parseInt(Math.random()*100000),
					success: function (req) {
						if(req.flag == true){
							yyxxbj = 1;
							yyxxmsg = req.msg;
...
//			obj.value = obj.value.replace(/[^A-Za-z0-9_]/g, "");
		...
		this.yzreservation = function(IDnumber){
			yzyyxx(IDnumber);
		}
