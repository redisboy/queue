...
//						  	var tempCapt ="";
						  	if (0 == str) {
							    ...
							    Born = Born.substring(0,4)+Born.substring(5,7)+Born.substring(8,10);
							    Address = readSysCard.Address;//地址
							    Police = readSysCard.IssuedAt;//发证机关
							    UserLifeB = readSysCard.EffectedDate;//有效期开始
							    UserLifeE = readSysCard.ExpiredDate;//有效期结束
							    //SynCardOcx1.SetPhotoType(2);//用base64
							    //验证黑名单
								gn.validateBlack(number);
								if(hmdbj == 1){
									if(confirm("此人在黑名单，确定打印吗？")){
										var params = "base64picPhoto=" + encodeURIComponent(readSysCard.Picture) + "&cardNumber="+number;
										...
										window.open("idCardPrintGPY.jsp?nameA="+encodeURIComponent(encodeURIComponent(NameA,'utf-8'))+"&number="+number+"&Sex="+encodeURIComponent(encodeURIComponent(Sex,'utf-8'))+"&Nation="+encodeURIComponent(encodeURIComponent(Nation,'utf-8'))+"&Born="+Born+"&Address="+encodeURIComponent(encodeURIComponent(Address,'utf-8'))+"&Police="+encodeURIComponent(encodeURIComponent(Police,'utf-8'))+"&UserLifeB="+UserLifeB+"&UserLifeE="+encodeURIComponent(encodeURIComponent(UserLifeE,'utf-8')), "_blank", features);   
													}
										}); 
									hmdbj = 0;
									}else{
									...
									var params = "base64picPhoto=" + encodeURIComponent(readSysCard.Picture) + "&cardNumber="+number;
										$.ajax({type:"POST", cache:false, data:params,
												...
												var features = "status=no,resizable=no,top=0,left=0,scrollbars=no"
															+ "titlebar=no,menubar=no,location=no,toolbar=no,z-look=yes"
										window.open("idCardPrintGPY.jsp?nameA="+encodeURIComponent(encodeURIComponent(NameA,'utf-8'))+"&number="+number+"&Sex="+encodeURIComponent(encodeURIComponent(Sex,'utf-8'))+"&Nation="+encodeURIComponent(encodeURIComponent(Nation,'utf-8'))+"&Born="+encodeURIComponent(encodeURIComponent(Born,'utf-8'))+"&Address="+encodeURIComponent(encodeURIComponent(Address,'utf-8'))+"&Police="+encodeURIComponent(encodeURIComponent(Police,'utf-8'))+"&UserLifeB="+UserLifeB+"&UserLifeE="+encodeURIComponent(encodeURIComponent(UserLifeE,'utf-8')), "_blank", features);   
													}
										}); 
								}
