...
function submitCheck(){
	...
	var deptHall=document.getElementById("deptHall");
	var deptCodeContent;
	var deptHallContent;
	if(deptCode){
		for(var i=0;i<deptCode.length;i++){
			if(deptCode[i].selected==true){
				deptCodeContent=deptCode[i].text;
			}
		}
		if(deptCodeContent=="请选择部门"){
			alert("请选择部门");
		}else if(deptHall){
			for(var i=0;i<deptHall.length;i++){
				if(deptHall[i].selected==true){
					deptHallContent=deptHall[i].text;
				}
			}
			if(deptHallContent=="请选择大厅"){
				alert("请选择大厅");
			}else{
				toDo();
			}
...
if(urlFlag==17){// 叫号间隔时间统计
  document.form1.action = "jhsjtj.action?tjms=1";
  }
...
function deptCodeChange(){
	...
	if(deptCode==''){
		...
		$('#depthalldiv').show();
		$.ajax({
			url:"findDepthallbyDeptcode2.action",
			async:false,
			cache:false,
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">部门大厅选择</span>
    </td>
    ...
    <form action="" method="post" name="form1">
     <input name="flag" id="flag" type="hidden" value="1"/>
        ...
        <ww:if test="${role eq 1}">
          ...
          <br>
          <input name="button" type="button" class="button" value="确定" onClick="submitCheck()"/>
          <br/>
	</form>
