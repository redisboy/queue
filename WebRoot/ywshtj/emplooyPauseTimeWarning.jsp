...
<%
	...
	%>
		window.onload=function(){
			...
			<%}%>
				str2 += "<set value='<%=count.getPausetime() %>' />";
			<%if(list.size()>1){%>
				sumAvg += "<set value='<%=count.getSumAvg()%>' />";
			<%}}%>
			var strXML = "<graph rotateYAxisName='0' caption='暂停时间预警'  showToolTip='0' showSum='1' shownames='1' showvalues='1' showSum='1' decimalPrecision='1' formatNumberScale='1' useRoundEdges='1'>"
			strXML += "<categories>"+str1+"</categories>";
			strXML += "<dataset seriesName='暂停时间' color='' showValues='' showSum='1'>"+str2+"</dataset>";
...
function query() { 
	...
	<%if(flag.equals("0")){ %>
	document.form1.action = "getEmplooyPauseTimeWarning.action?flag=0";
	<%}else{%>
	document.form1.action = "getEmplooyPauseTimeWarning.action?flag=1";
	<%}%>
	document.form1.submit();
}
function hpExport(obj){
	window.location.href="/queue/ywshtj/emplooyPauseTimeWarning_Export_excel.jsp";
    return true;
}
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">暂停时间预警</span>
    </td>
  ...
  <tr>
	...
	<% if("0".equals(flag)){ %>  
		<ec:table items="list" var="list" 
					action="getEmplooyPauseTimeWarning.action?flag=0"
					editable="false" 
					width="100%" 
					listWidth="100%"
					classic="true"
				...
				<ec:row style="text-align:center">
					<ec:column width="10%" property="code" title="编号" style="text-align:center" editable="false" />
					<ec:column width="10%" property="name" title="姓名" style="text-align:center" editable="false" />
					<ec:column width="8%" property="deptcode" title="所属部门编号" style="text-align:center" editable="false" />
					<ec:column width="10%" property="pausetime" title="暂停时间" style="text-align:center" editable="false" />
				</ec:row>
				<c:if test="${msg != null}">
				<ec:extendrow>
	...
	<%}}else{ %>
	<ec:table items="list" var="list" 
					action="getEmplooyPauseTimeWarning.action?flag=1"
					editable="false" 
					width="100%" 
					listWidth="100%"
					classic="true"
