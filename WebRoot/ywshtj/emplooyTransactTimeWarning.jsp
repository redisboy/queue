...
<%
	List<YwShTj> list = (List<YwShTj>) request.getAttribute("listWaring");
	...
	%>
		window.onload=function(){
			var str1 = "", str2="", str3="", str4="",sumAvg="";
			<% for (YwShTj qwc : list) {
			if(flag.equals("0")){
			%>
			str1 += "<category label='<%=qwc.getName()%>' />";
			<% }else{%>
			str1 += "<category label='<%=qwc.getDeptcode()%>' />";
			<%}%>
			str4 += "<set value='<%=qwc.getAve_worktime()%>' />";
			<% //判断预警值，如果List值是一个则不显示
			if(list.size()>1){
			%>
			sumAvg += "<set value='<%=qwc.getSumAvg()%>' />";
			<%}}%>
			var strXML = "<graph rotateYAxisName='0' caption='员工办理业务时间考核'  showToolTip='0' showSum='1' shownames='1' showvalues='1' showSum='1' decimalPrecision='1' formatNumberScale='1' useRoundEdges='1'>"
			strXML += "<categories>"+str1+"</categories>";
			strXML += "<dataset seriesName='平均办理时间' color='' showValues='' showSum='1'>"+str4+"</dataset>";
			strXML += "<dataset seriesName='预警值' color='FF0000' showValues='' parentYAxis='S'>"+sumAvg+"</dataset></graph>"
		
		    var chart1 = new FusionCharts("/queue/plugs/FusionCharts/MSColumnLine3D.swf","chart1ID","800","400","0","0");
...
function query() { 
	...
	<%if(flag.equals("0")){ %>
	document.form1.action = "getEmplooyTransactTimeWarning.action?flag=0";
	<%}else{%>
	document.form1.action = "getEmplooyTransactTimeWarning.action?flag=1";
	<%}%>
	document.form1.submit();
}
function hpExport(obj){
	window.location.href="/queue/ywshtj/emplooyTransactTimeWarning_Export_excel.jsp";
    return true;
}
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">办理业务时间预警</span>
    </td>
  ...
  <tr>
	...
	<% if(flag.equals("0")){ %> 
		<ec:table items="list" var="list" 
					action="getEmplooyTransactTimeWarning.action?flag=0"
					editable="false" 
					width="100%" 
					listWidth="100%"
					classic="true"
				...
				<ec:row style="text-align:center">
					...
					<ec:column width="10%" property="name" title="员工姓名" style="text-align:center" editable="false" />
					<ec:column width="8%" property="deptcode" title="部门编号" style="text-align:center" editable="false" />
	...
	<%}}else{ %>
	<ec:table items="list" var="list" 
					action="getEmplooyTransactTimeWarning.action?flag=1"
					editable="false" 
					width="100%" 
					listWidth="100%"
					classic="true"
