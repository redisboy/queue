...
$(function(){
	...
	var user_bkblyw = $("#user_bkblyw").val();
	if (undefined != user_bkblyw && "" != user_bkblyw) {
		var datas = user_bkblyw.split(',');
		$('input[name=bkblyw]').each(function(i) {
	...
	var yw_dybd = $("#yw_dybd").val();
	if (undefined != yw_dybd && "" != yw_dybd) {
		var datas = yw_dybd.split('H');
		$('input[name=dybd]').each(function(i) {
   	    	for (var j = 0, len = datas.length; j < len; ++ j) {
	   			if (datas[j] == $(this).val()){
	   				$(this).attr('checked','true');
	   				break;
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">业务类型修改</span>
    </td>
    <td width="3" height="51"><img src="/queue/images/t-3.jpg" width="8" height="51"></td>
  </tr>
  <tr>
    <td width="7" background="/queue/images/t-4.jpg">&nbsp;
    	<input type="hidden" id="user_bkblyw" value="${user_bkblyw}"/>
    	<input type="hidden" id="yw_dybd" value="${yw_dybd}"/>
    </td>
    ...
    <table width="40%" border="0" cellpadding="1" cellspacing="1" class="table_back">
      <tr>
       <input type="hidden" id="id" name="id"  value="${id }" />
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
          <input name="business" id="business" maxlength="20" style="width:200px" value="${name }"  />
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
           <select name="queueCode" id="queueCode" >
             <c:forEach var="Queue" items="${list}" varStatus="vs">
                 <option value="${ Queue.code}" <c:if test="${Queue.code==queueCode }">selected</c:if>>${ Queue.name}</option>
             </c:forEach>
           </select>
        </td>
      </tr>
      ...
      <tr>
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
          <select id="liuzhuan" name="liuzhuan">
              ...
              <c:forEach var="bus" items="${businessListVo}" varStatus="vs">
                    <option value="${ bus.id}" <c:if test="${bus.id==liuzhuan}">selected</c:if>>${bus.name}</option>
             </c:forEach>
          </select>
          &nbsp;
          <input type="radio" id="isautolz" name="isautolz" value="0" <c:if test="${isautolz eq 0}">checked</c:if>/>自动
		  <input type="radio" id="isautolz" name="isautolz" value="1" <c:if test="${isautolz eq 1}">checked</c:if>/>手动
        </td>
      </tr>
      <tr>
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
        	<select id="isOpenTztd" name="isOpenTztd">
              <option value="0" <c:if test="${isOpenTztd==0 }">selected</c:if>>是</option>
              <option value="1" <c:if test="${isOpenTztd==1 }">selected</c:if>>否</option>
          	</select>
        </td>
      </tr>
      <tr>
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
        	<select id="isOpenZhiWen" name="isOpenZhiWen">
              <option value="0" <c:if test="${isOpenZhiWen==0 }">selected</c:if>>是</option>
              <option value="1" <c:if test="${isOpenZhiWen==1 }">selected</c:if>>否</option>
          	</select>
        </td>
      </tr>
      <tr>
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
        	<select id="outflag" name="outflag">
              <option value="0" <c:if test="${outflag==0 }">selected</c:if>>是</option>
              <option value="1" <c:if test="${outflag==1 }">selected</c:if>>否</option>
          	</select>
        </td>
      </tr>
      <tr>
        <td width="10%" height="35" bgcolor="#FFFFFF" class="tableheader1">每日放号数量</td>
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
        	<input id="ywl" name="ywl" value="${ywl}" style="width:40px"/>
      ...
      <tr>
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
        	<input id="managemin" name="managemin" value="${managemin}" style="width:40px"/>
        </td>
      </tr>
      <tr>
        <td width="10%" height="35" bgcolor="#FFFFFF" class="tableheader1">帮助信息</td>
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
        	<textarea id="help_info" name="help_info" style="width:80%;height:120px">${help_info}</textarea>
        </td>
      </tr>
       <tr>
...
<!--         	<input id="yyywmc" name="yyywmc" value="${yyywmc}" style="width:200px"/> -->
        	<textarea id="yyywmc" name="yyywmc" style="width:80%;height:120px">${yyywmc}</textarea>
        </td>
      </tr>
      <tr>
        <td width="10%" height="35" bgcolor="#FFFFFF" class="tableheader1">打印表单业务名称</td>
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
          <input name="bdywmc" id="bdywmc" maxlength="100" style="width:200px" value="${bdywmc}"   />
    ...
    <br>
      <br>
	  <input name="bt1" class="button" type="button" value=" 更改 " onClick="updateBusinessByID()"/>
...
<script type="text/javascript">
	function updateBusinessByID(){
      ...
      document.form1.action = "updateByID.action";
	  document.form1.submit();
	}
	toSelected("flag", "${flag}");
</script>
</html>
