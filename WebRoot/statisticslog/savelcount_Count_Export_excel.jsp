...
<%
	List<SaveLCount> list =  (List<SaveLCount>) request.getSession().getAttribute("list");
	response.reset();
	response.setContentType("application/vnd.ms-excel;charset=UTF-8");
	response.setHeader("Content-Disposition", "attachment;filename="
			+ new String("SaveLogCount.xls".getBytes("gb2312"), "iso8859-1"));
	...
	headerFormat.setVerticalAlignment(VerticalAlignment.CENTRE);
		...
		ws.addCell(new Label(1, 0, "安全审计", headerFormat));
		ws.addCell(new Label(2, 0, "数量", headerFormat));
		for (int i = 0; i < list.size(); i++) {
			ws.addCell(new Label(0, i + 1, list.get(i).getUserName(),
					headerFormat));
			ws.addCell(new Label(1, i + 1, list.get(i).getSaveCel(),
					headerFormat));
			ws.addCell(new Label(2, i + 1, list.get(i).getSaveCount(),
					headerFormat));
		}
		for (int i = 0; i < 2; i++) {
			int count = 1;
