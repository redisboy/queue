...
<body onload="printID()">
	<div id="idcard" class="sfz">
		...
		<%if("长期".equals(URLDecoder.decode(request.getParameter("UserLifeE"),"utf-8").substring(0, 2))){%>
			<div class="div10">
				<%=request.getParameter("UserLifeB").substring(0,4) %>.<%=request.getParameter("UserLifeB").substring(4,6) %>.<%=request.getParameter("UserLifeB").substring(6,8) %>-<%=URLDecoder.decode(request.getParameter("UserLifeE"),"utf-8") %>
			</div>
		<%}else{%>
			<div class="div10">
				<%=request.getParameter("UserLifeB").substring(0,4) %>.<%=request.getParameter("UserLifeB").substring(4,6) %>.<%=request.getParameter("UserLifeB").substring(6,8) %>-<%=request.getParameter("UserLifeE").substring(0,4) %>.<%=request.getParameter("UserLifeE").substring(4,6) %>.<%=request.getParameter("UserLifeE").substring(6,8) %>
			</div>
		<%} %>
