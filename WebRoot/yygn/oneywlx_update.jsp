...
<script type="text/javascript">
	...
	function updateOneywlx() {
		var yysl=$("#yysl").val();
		var dtId=$("#dtId").val();
		document.form1.action = "updateOneywlx.action?yysl="+yysl+"&dtId="+dtId; 
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">修改一级业务类型</span>
    </td>
  ...
  <tr>
    ...
    <table width="69%" border="0" cellpadding="1" cellspacing="1" class="table_back">
      <tr>
        ...
        <td width="30%" height="35" bgcolor="#F1F9FD">&nbsp;
          <input name="name" id="name" maxlength="20" style="width:200px"  value="${list.name }"   />
      ...
      <tr id="roletr">
		...
		<td width="80%" height="35" bgcolor="#F1F9FD">
			&nbsp;
			<input type="radio" name="xtlb" id="xtlb" value="1" <c:if test="${list.xtlb eq 1}">checked</c:if> />机动车
			&nbsp;
			<input type="radio" name="xtlb" id="xtlb" value="2" <c:if test="${list.xtlb eq 2}">checked</c:if>  />驾驶证
			&nbsp;
			<input type="radio" name="xtlb" id="xtlb" value="3" <c:if test="${list.xtlb eq 3}">checked</c:if>  />违法
			&nbsp;
		</td>
	  </tr>
	  <tr id="roletr">
		...
		<td width="80%" height="35" bgcolor="#F1F9FD">
			...
			<%-- 这里考虑到选择一个按钮有隐藏显示功能， 所以我觉得不能做动态选择， 所以给他默认为 不包含二级业务类型！
			<input type="radio" name="ejywzt" id="ejywzt" value="0" onclick="show('0')" <c:if test="${list.ejywzt eq 0}">checked</c:if> />是
			&nbsp;
			<input type="radio" name="ejywzt" id="ejywzt" value="1"  onclick="show('1')" <c:if test="${list.ejywzt eq 1}">checked</c:if>  />否
			 --%>
			 
			<input type="radio" name="ejywzt" id="ejywzt" value="0" onclick="show('0')" />是
			&nbsp;
			<input type="radio" name="ejywzt" id="ejywzt" value="1"  onclick="show('1')" checked  />否
			&nbsp;
		</td>
		</tr>
    ...
    <br>
      <br>
	  <input name="bt1" class="button" type="button" value="修改 " onClick="updateOneywlx()"/>
