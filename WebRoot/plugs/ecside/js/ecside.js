...
Me.messageBar=null;
Me.messageBarCore=null;
Me.showMessageBar=function(message){
	Me.messageBar=document.getElementById(Me.id+"_messageBar");
	Me.messageBar.style.display="block";
	Me.messageBar.setAttribute("big","false");
	Me.messageBarCore=document.getElementById(Me.id+"_messageBar");	
	Me.messageBarCore.innerHTML=message;
};

Me.hiddenMessageBar=function(){
	Me.messageBar=document.getElementById(Me.id+"_messageBar");
	Me.messageBar.style.display="none";
};
...
Me.updateCallBack=function(responseObj){
	...
	for (var i=1;i<rs.length ;i+=3 ){
		...
		if (resultCode=="1" || resultCode=="Success") {
			//Success
			Me.showMessageBar(ECSideMessage.SUCCESS_MSG);
			Me.messageBar.className="succMessage";
			ECSideUtil.getRemoveUpdatedClassRows(ecsideObj.forUpdateRows,recordKey);
		}else{
			//Fail
			Me.showMessageBar(ECSideMessage.FAIL_MSG);
			Me.messageBar.className="failMessage";
			if (message.length>1){
				alert(message);
			}
		}
...
ECSideUtil.saveGirdBatch=function(buttonObj,formid,isBatch){
	...
	ECSideUtil.getGridObj(formid).hiddenMessageBar();
