...
<%
	List<Security> list =  (List<Security>) request.getSession().getAttribute("list");
	response.reset();
	response.setContentType("application/vnd.ms-excel;charset=UTF-8");
	response.setHeader("Content-Disposition", "attachment;filename="
			+ new String("SercurityLog.xls".getBytes("gb2312"), "iso8859-1"));

	OutputStream os = response.getOutputStream();

	WritableWorkbook wwb = null;
	wwb = Workbook.createWorkbook(os);//将 WritableWorkbook 写入到输出流  
	WritableSheet ws = wwb.createSheet("安全日志查询", 0);//创建第一个sheet  
	...
	headerFormat.setVerticalAlignment(VerticalAlignment.CENTRE);
		...
		ws.addCell(new Label(3, 0, "事件", headerFormat));
		ws.addCell(new Label(4, 0, "结果描述", headerFormat));
		ws.addCell(new Label(5, 0, "入库时间", headerFormat));
		for (int i = 0; i < list.size(); i++) {
			...
			ws.addCell(new Label(2, i + 1, list.get(i).getOriginIp(),
					headerFormat));
			ws.addCell(new Label(3, i + 1, list.get(i).getEvent(),
					headerFormat));
			ws.addCell(new Label(4, i + 1, list.get(i).getResultDepict(),
					headerFormat));
			ws.addCell(new Label(5, i + 1, list.get(i).getrTime(),
					headerFormat));
		}
		for (int i = 0; i < 2; i++) {
			int count = 1;
