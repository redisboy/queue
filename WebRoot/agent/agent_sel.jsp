...
function query() { 
	...
	if(reg.test(id)||id==""){
		document.form1.action = "dlrdj.action";
...
function add(){
	document.form1.action="dlrdjadd.action";
	document.form1.submit();
}
function delteAgent(id){
	if(confirm("确定删除吗?")){ 
		document.forms.action="agentDel.action?id="+id;
		document.forms.submit();
	}
}
window.onload=function(){
	var tableBody=document.getElementById("ec_table_body"); //得到tablebody
	var child=tableBody.childNodes;     //得到tr节点数组
	
	for(var i=0;i<child.length;i++){
		var lastTd=child[i].lastChild;    //遍历每一个tr下的最后一个td节点
		lastTd.innerHTML="<input id='print"+i+"' type='button' value='打印' onclick='print("+i+")' />" ;  //对每个td节点添加按钮和打印事件
	}
}

function print(a){
	var printId="print"+a;
	var tr=document.getElementById(printId).parentNode.parentNode;
	var id=tr.firstChild.innerText;
	window.location.href="/queue/dlrgl/agentPrint.action?id="+id;
}
...
<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <td height="51" valign="middle"  background="/queue/images/t-2.jpg">
    	<span style="font-family:'黑体', '宋体';font-size:18px; color:#fff;padding-left:15px;letter-spacing: 10px;letter-spacing: 10px;">代理人信息</span>
    </td>
     ...
     <input name="flag" id="flag" type="hidden" value="1"/>
          <table width="80%" border="0" cellpadding="1" cellspacing="1" class="table_back">
            ...
            <td class=tableheader1 width=10% height=30>身份证:</td>
          	<td width=150 bgColor=#f1f9fd height=30>&nbsp;
	          <input name="idCard" id="idCard" type="text" maxlength="18" class="inputface" value=""/>
           </td>
	...
	<td> 
		<ec:table items="list" var="list" 
				...
				<ec:row style="text-align:center">
					<ec:column width="5%" property="id" title="编号" style="text-align:center" editable="false" />
					...
					<ec:column width="8%" property="register_date" title="初次登记日期" style="text-align:center" editable="false" />
                    ...
                    <ec:column width="8%" property="validity" title="有效期" style="text-align:center" editable="false" />
                    <ec:column width="7%" property="status" title="状态" style="text-align:center" editable="false" />
                    <ec:column width="10%" property="upd" title="修改" style="text-align:center" editable="false" />
                    <ec:column width="10%" property="del" title="删除" style="text-align:center" editable="false" />
                    <ec:column width="10%" property="print" title="打印" style="text-align:center" editable="false"  />
				</ec:row>
				
	</ec:table><br>
</td>
	</tr><!-- <ec:column width="10%" property="del" title="操作" style="text-align:center" editable="false" /> -->
	</table>
