...
(function () {
	var h = YAHOO.lang, a = Object.prototype, b = "[object Array]", g = "[object Function]", c = "[object Object]", e = [], d = ["toString", "valueOf"], f = {isArray:function (i) {
		...
		for (t = n.length - 1; t >= 0; t = t - 1) {
			i = i.replace(new RegExp("~-" + t + "-~"), "{" + n[t] + "}", "g");
		}
		return i;
	}, trim:function (j) {
		try {
		...
		return (h.isObject(i) || h.isString(i) || h.isNumber(i) || h.isBoolean(i));
	}};
...
(function () {
	...
	ao.Dom = {CUSTOM_ATTRIBUTES:(!z.hasAttribute) ? {"for":"htmlFor", "class":an} : {htmlFor:"for", className:aK}, DOT_ATTRIBUTES:{}, get:function (f) {
		var c, a, e, g, d, b;
		if (f) {
			if (f[aF] || f.item) {
				return f;
		...
		if (!a) {
			...
			if ((b[0] !== null && h[0] != b[0]) || (b[1] !== null && h[1] != b[1])) {
				ao.Dom._setXY(i, {pos:b, noRetry:true});
			}
		}
	}, setX:function (b, a) {
...
(function () {
	var v = YAHOO.util, w = "clientTop", r = "clientLeft", n = "parentNode", m = "right", a = "hasLayout", o = "px", c = "opacity", l = "auto", t = "borderLeftWidth", q = "borderTopWidth", h = "borderRightWidth", b = "borderBottomWidth", e = "visible", g = "transparent", j = "height", s = "width", p = "style", d = "currentStyle", f = /^width|height$/, i = /^(\d[.\d]*)+(em|ex|px|gd|rem|vw|vh|vm|ch|mm|cm|in|pt|pc|deg|rad|ms|s|hz|khz|%){1}?/i, k = {get:function (A, y) {
		var z = "", x = A[d][y];
		if (y === c) {
...
(function () {
	...
	b.Dom.Color = {KEYWORDS:{black:"000", silver:"c0c0c0", gray:"808080", white:"fff", maroon:"800000", red:"f00", purple:"800080", fuchsia:"f0f", green:"008000", lime:"0f0", olive:"808000", yellow:"ff0", navy:"000080", blue:"00f", teal:"008080", aqua:"0ff"}, re_RGB:/^rgb\(([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\)$/i, re_hex:/^#?([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})$/i, re_hex3:/([0-9A-F])/gi, toRGB:function (e) {
...
YAHOO.util.CustomEvent.prototype = {subscribe:function (d, c, b) {
	if (!d) {
...
if (!YAHOO.util.Event) {
	YAHOO.util.Event = function () {
		var h = false, g = [], e = [], d = 0, j = [], c = 0, b = {63232:38, 63233:40, 63234:37, 63235:39, 63276:33, 63277:34, 25:9}, a = YAHOO.env.ua.ie, i = "focusin", f = "focusout";
		return {POLL_RETRYS:500, POLL_INTERVAL:40, EL:0, TYPE:1, FN:2, WFN:3, UNLOAD_OBJ:3, ADJ_SCOPE:4, OBJ:5, OVERRIDE:6, CAPTURE:7, lastError:null, isSafari:YAHOO.env.ua.webkit, webkit:YAHOO.env.ua.webkit, isIE:a, _interval:null, _dri:null, _specialTypes:{focusin:(a ? "focusin" : "focus"), focusout:(a ? "focusout" : "blur")}, DOMReady:false, throwErrors:false, startInterval:function () {
			...
			for (var k = 0; k < l.length; k = k + 1) {
				j.push({id:l[k], fn:q, obj:o, overrideContext:n, checkReady:p});
			}
			d = this.POLL_RETRYS;
			this.startInterval();
		...
		}, DOMReadyEvent:new YAHOO.util.CustomEvent("DOMReady", YAHOO, 0, 0, 1), _load:function (k) {
			if (!h) {
				h = true;
				...
				if (k) {
					for (var n = 0, l = k.length; n < l; ++n) {
						...
						if (q && q[this.EL] === m && (!t || t === q[this.TYPE])) {
							o.push({type:q[this.TYPE], fn:q[this.FN], obj:q[this.OBJ], adjust:q[this.OVERRIDE], scope:q[this.ADJ_SCOPE], index:n});
						}
					}
				}
			}
...
YAHOO.util.EventProvider.prototype = {__yui_events:null, __yui_subscribers:null, subscribe:function (a, e, b, c) {
...
(function (b) {
	var a = YAHOO.lang;
	a.augmentObject(b, {version:"0.1", lang:{}, mods:{}, plugins:{}, add:function (c, e, d) {
		this.mods[c] = {name:c, fn:e, details:d || {}};
		return this;
		...
		for (g = 0, c = d.length; g < c; ++g) {
			h = d[g];
			if (!e[h]) {
				e[h] = a.merge(j, {name:h});
			}
		}
	}, _isReady:false, _setup:function () {
		this._loadModules();
		this._isReady = true;
	}, _attached:{}, _loadModules:function () {
...
KISSY.Editor.add("core~dom", function (b) {
	...
	b.Dom = {getText:function (c) {
		...
		return g;
	}, BLOCK_ELEMENTS:{blockquote:1, div:1, h1:1, h2:1, h3:1, h4:1, h5:1, h6:1, hr:1, p:1, address:1, center:1, pre:1, form:1, fieldset:1, caption:1, table:1, tbody:1, tr:1, th:1, td:1, ul:1, ol:1, dl:1, dt:1, dd:1, li:1}};
	if (a.ie) {
		b.Dom.getText = function (c) {
...
KISSY.Editor.add("core~color", function (d) {
	...
	d.Color = {KEYWORDS:{black:"000", silver:"c0c0c0", gray:"808080", white:"fff", maroon:"800000", red:"f00", purple:"800080", fuchsia:"f0f", green:"008000", lime:"0f0", olive:"808000", yellow:"ff0", navy:"000080", blue:"00f", teal:"008080", aqua:"0ff"}, re_RGB:/^rgb\(([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\)$/i, re_hex:/^#?([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})$/i, re_hex3:/([0-9A-F])/gi, toRGB:function (e) {
		...
		if (this.re_RGB.exec(i)) {
			var h = (b.$1 >> 0)[c](16), f = (b.$2 >> 0)[c](16), e = (b.$3 >> 0)[c](16);
			i = [h.length == 1 ? "0" + h : h, f.length == 1 ? "0" + f : f, e.length == 1 ? "0" + e : e].join("");
		}
		if (i.length < 6) {
...
KISSY.Editor.add("core~command", function (f) {
	var d = YAHOO.env.ua, b = {backColor:d.gecko ? "hiliteColor" : "backColor"}, c = "bold,italic,underline,strikeThrough", a = "styleWithCSS", e = "execCommand";
	f.Command = {exec:function (i, h, j, g) {
		h = b[h] || h;
		this._preExec(i, h, g);
		i[e](h, false, j);
...
KISSY.Editor.add("core~range", function (b) {
	...
	b.Range = {getSelectionRange:function (f) {
		...
		return c.startContainer || (c.parentElement && c.parentElement()) || (c.commonParentElement && c.commonParentElement());
...
KISSY.Editor.add("core~instance", function (k) {
	...
	f.augmentObject(k.Instance.prototype, {_init:function () {
		this._renderUI();
		this._initPlugins();
		this._initAutoFocus();
	}, _renderUI:function () {
		...
		var n = this.textarea, r = e.getRegion(n), p = (r.right - r.left - 2) + "px", l = (r.bottom - r.top - 2) + "px", m = document.createElement("div"), q, o;
		m.className = d;
		m.style.width = p;
		m.innerHTML = h;
		q = m.childNodes[1];
		...
		var p = this.contentDoc, o = this.config, l = "content" + (o.debug ? "" : "-min") + ".css", n = o.base + j + "/" + o.theme + "/" + l, m = this;
		p.open();
		p.write(b.replace("{CONTENT_CSS}", n).replace("{CONTENT}", this.textarea.value));
		p.close();
		if (a) {
		...
		var n = this.contentDoc.body.lastChild, l = k.Range.getSelectionRange(this.contentWin);
		if (g.ie) {
			try {
			...
			try {
				l.setEnd(n, n.childNodes.length);
			}
			catch (m) {
			}
			l.collapse(false);
...
KISSY.Editor.add("core~toolbar", function (j) {
	...
	e.augmentObject(j.Toolbar.prototype, {init:function () {
		...
		if (!v.lang) {
			v.lang = e.merge(w.common, this.lang[v.name] || {});
		}
		s = m.replace("{TITLE}", v.lang.title || "").replace("{NAME}", v.name).replace("{TEXT}", v.lang.text || "");
		if (n) {
			s = s.replace("outer-box", "outer-box ks-inline-block").replace("inner-box", "inner-box ks-inline-block");
		}
		k.innerHTML = s;
		v.domEl = u = k.firstChild;
		if (t == h.TOOLBAR_MENU_BUTTON || t == h.TOOLBAR_SELECT) {
		...
		var t = u.domEl, s = t.getElementsByTagName("span")[0].parentNode;
		b.addClass(t, q);
		s.innerHTML = l.replace("{NAME}", u.name).replace("{TEXT}", u.lang.text || "");
		...
		r.on(s, "mouseout", function (w) {
			var v = r.getRelatedTarget(w), u;
			try {
				if (s.contains) {
					u = s.contains(v);
				} else {
					if (s.compareDocumentPosition) {
						u = s.compareDocumentPosition(v) & 8;
					}
				}
			}
			catch (w) {
		...
		if (n) {
			r.on(s, "mouseenter", function () {
				b.addClass(s, a);
			});
			r.on(s, "mouseleave", function () {
				b.removeClass(s, a);
			});
		}
		...
		try {
			if (t.queryCommandEnabled(u.name)) {
				if (t.queryCommandState(u.name)) {
					b.addClass(u.domEl, p);
				} else {
					b.removeClass(u.domEl, p);
				}
			}
		}
		catch (s) {
...
KISSY.Editor.add("core~statusbar", function (d) {
	var e = YAHOO.util, c = YAHOO.lang, b = YAHOO.env.ua.ie, a = "<div class=\"ks-editor-stripbar-sep kissy-inline-block\"></div>", g = "<div class=\"ks-editor-statusbar-item ks-editor-{NAME} ks-inline-block\"></div>", f = document.createElement("div");
	d.Statusbar = function (h) {
		this.editor = h;
		this.config = h.config;
		this.lang = d.lang[this.config.language];
	};
	c.augmentObject(d.Statusbar.prototype, {init:function () {
		...
		var h, j = this.lang;
		if (!i.lang) {
			i.lang = c.merge(j.common, this.lang[i.name] || {});
		}
		f.innerHTML = g.replace("{NAME}", i.name);
		...
		if (b) {
			h = d.Dom.setItemUnselectable(h);
		}
		this.domEl.appendChild(h);
	}});
});
KISSY.Editor.add("core~menu", function (n) {
	var e = YAHOO.util, i = e.Dom, m = e.Event, j = YAHOO.env.ua, g = "display", k = "none", f = "", a = "ks-editor-drop-menu", l = "ks-editor-drop-menu-shadow", d = "ks-editor-drop-menu-content", c = "ks-editor-toolbar-button-selected", b = a + "-shim", h;
	n.Menu = {generateDropMenu:function (q, p, s) {
		var r = document.createElement("div"), o = this;
		r.innerHTML = "<div class=\"" + l + "\"></div><div class=\"" + d + "\"></div>";
		r.className = a;
		r.style[g] = k;
		...
		m.on([document, q.contentDoc], "click", function () {
			if (q.activeDropMenu) {
...
KISSY.Editor.add("smilies~config~default", function (a) {
	a.Smilies = a.Smilies || {};
	a.Smilies["default"] = {name:"default", mode:"icons", cols:5, fileNames:["smile", "confused", "cool", "cry", "eek", "angry", "wink", "sweat", "lol", "stun", "razz", "shy", "rolleyes", "sad", "happy", "yes", "no", "heart", "idea", "rose"], fileExt:"gif"};
});
KISSY.Editor.add("smilies~config~wangwang", function (a) {
	a.Smilies = a.Smilies || {};
	a.Smilies.wangwang = {name:"wangwang", mode:"sprite", base:"http://a.tbcdn.cn/sys/wangwang/smiley/48x48/", spriteStyle:"background: url(http://a.tbcdn.cn/sys/wangwang/smiley/sprite.png) no-repeat -1px 0; width: 288px; height: 235px", unitStyle:"width: 24px; height: 24px", filePattern:{start:0, end:98, step:1}, fileExt:"gif"};
});
KISSY.Editor.add("plugins~base", function (b) {
	var c = b.PLUGIN_TYPE, a = "bold,italic,underline,strikeThrough,insertOrderedList,insertUnorderedList";
	b.addPlugin(a.split(","), {type:c.TOOLBAR_BUTTON, exec:function () {
		this.editor.execCommand(this.name);
		this.editor.toolbar.updateState();
	}});
});
KISSY.Editor.add("plugins~color", function (n) {
	var c = YAHOO.util, h = c.Dom, m = c.Event, l = YAHOO.env.ua, e = l.ie, k = n.PLUGIN_TYPE, d = "<div class=\"ks-editor-palette-table\"><table><tbody>{TR}</tbody></table></div>", b = "<td class=\"ks-editor-palette-cell\"><div class=\"ks-editor-palette-colorswatch\" title=\"{COLOR}\" style=\"background-color:{COLOR}\"></div></td>", i = ["000", "444", "666", "999", "CCC", "EEE", "F3F3F3", "FFF"], g = ["F00", "F90", "FF0", "0F0", "0FF", "00F", "90F", "F0F"], f = ["F4CCCC", "FCE5CD", "FFF2CC", "D9EAD3", "D0E0E3", "CFE2F3", "D9D2E9", "EAD1DC", "EA9999", "F9CB9C", "FFE599", "B6D7A8", "A2C4C9", "9FC5E8", "B4A7D6", "D5A6BD", "E06666", "F6B26B", "FFD966", "93C47D", "76A5AF", "6FA8DC", "8E7CC3", "C27BAD", "CC0000", "E69138", "F1C232", "6AA84F", "45818E", "3D85C6", "674EA7", "A64D79", "990000", "B45F06", "BF9000", "38761D", "134F5C", "0B5394", "351C75", "741B47", "660000", "783F04", "7F6000", "274E13", "0C343D", "073763", "20124D", "4C1130"], j = "ks-editor-palette-colorswatch", a = "ks-editor-palette-cell-selected";
	n.addPlugin(["foreColor", "backColor"], {type:k.TOOLBAR_MENU_BUTTON, color:"", _indicator:null, swatches:null, dropMenu:null, range:null, init:function () {
		var p = this.domEl, o = p.getElementsByTagName("span")[0].parentNode;
		this.color = this._getDefaultColor();
		h.addClass(p, "ks-editor-toolbar-color-button");
		o.innerHTML = "<div class=\"ks-editor-toolbar-color-button-indicator\" style=\"border-bottom-color:" + this.color + "\">" + o.innerHTML + "</div>";
		this._indicator = o.firstChild;
		this._renderUI();
		this._bindUI();
		this.swatches = h.getElementsByClassName(j, "div", this.dropMenu);
	}, _renderUI:function () {
		this.dropMenu = n.Menu.generateDropMenu(this.editor, this.domEl, [1, 0]);
		this._generatePalettes();
		if (e) {
			n.Dom.setItemUnselectable(this.dropMenu);
		}
	}, _bindUI:function () {
		this._bindPickEvent();
		m.on(this.domEl, "click", function () {
			this.range = this.editor.getSelectionRange();
			e && this.editor.contentDoc.selection.empty();
		...
		for (s = 0, p = q.length; s < p; ++s) {
			...
			r = n.Color.toRGB("#" + q[s]).toUpperCase();
		...
		var o = this;
		m.on(this.dropMenu, "click", function (q) {
			var r = m.getTarget(q), p = r.getAttribute("title");
			if (p && p.indexOf("RGB") === 0) {
				o._doAction(p);
			}
			m.stopPropagation(q);
			n.Menu.hideActiveDropMenu(o.editor);
		});
		...
		for (q = 0, o = p.length; q < o; ++q) {
			r = p[q];
			if (n.Color.toHex(r.style.backgroundColor) == s) {
				h.addClass(r.parentNode, a);
			} else {
				h.removeClass(r.parentNode, a);
			}
		}
...
KISSY.Editor.add("plugins~font", function (k) {
	var a = YAHOO.util, b = a.Dom, j = a.Event, f = YAHOO.env.ua, e = k.PLUGIN_TYPE, d = "ks-editor-option-hover", h = "<ul class=\"ks-editor-select-list\">{LI}</ul>", c = "<li class=\"ks-editor-option\" data-value=\"{VALUE}\"><span class=\"ks-editor-option-checkbox\"></span><span style=\"{STYLE}\">{KEY}</span></li>", i = "ks-editor-option-selected", g = {"10px":1, "13px":2, "16px":3, "18px":4, "24px":5, "32px":6, "48px":7};
	k.addPlugin(["fontName", "fontSize"], {type:e.TOOLBAR_SELECT, selectedValue:"", selectHead:null, selectList:null, options:[], items:null, selectedItem:null, range:null, init:function () {
		this.options = this.lang.options;
		this.selectHead = this.domEl.getElementsByTagName("span")[0];
		this._renderUI();
		this._bindUI();
	}, _renderUI:function () {
		this.selectList = k.Menu.generateDropMenu(this.editor, this.domEl, [1, 0]);
		this._renderSelectList();
		this.items = this.selectList.getElementsByTagName("li");
	}, _bindUI:function () {
		this._bindPickEvent();
		j.on(this.domEl, "click", function () {
			this.range = this.editor.getSelectionRange();
			f.ie && this.editor.contentDoc.selection.empty();
			if (this.selectedValue) {
				this._updateSelectedOption(this.selectedValue);
			} else {
				if (this.selectedItem) {
					b.removeClass(this.selectedItem, i);
		...
		for (m in l) {
			o = l[m];
			n += c.replace("{VALUE}", o).replace("{STYLE}", this._getOptionStyle(m, o)).replace("{KEY}", m);
		}
		this.selectList.innerHTML = h.replace("{LI}", n);
		b.addClass(this.selectList, "ks-editor-drop-menu-" + this.name);
	}, _bindPickEvent:function () {
		var l = this;
		j.on(this.selectList, "click", function (m) {
			var n = j.getTarget(m);
			if (n.nodeName != "LI") {
				n = b.getAncestorByTagName(n, "li");
			}
			if (!n) {
				return;
			}
			l._doAction(n.getAttribute("data-value"));
			j.stopPropagation(m);
			k.Menu.hideActiveDropMenu(l.editor);
		});
		if (f.ie === 6) {
			j.on(this.items, "mouseenter", function () {
				b.addClass(this, d);
			});
			j.on(this.items, "mouseleave", function () {
				b.removeClass(this, d);
			});
		}
		...
		this._updateHeadText(this._getOptionKey(l));
		...
		for (n = 0; n < l; ++n) {
			o = m[n];
			if (o.getAttribute("data-value") == p) {
				b.addClass(o, i);
				this.selectedItem = o;
			} else {
				b.removeClass(o, i);
		...
		try {
			if (p.queryCommandEnabled(l)) {
				...
				if (f.webkit && l == "fontSize") {
					q = this._getWebkitFontSize(q);
				}
				q && (o = this._getOptionKey(q));
				if (o in m) {
					if (q != this.selectedValue) {
						this.selectedValue = q;
						this._updateHeadText(o);
					}
				} else {
					this.selectedValue = "";
...
/*KISSY.Editor.add("plugins~image", function (j) {
	var c = YAHOO.util, b = c.Dom, u = c.Event, l = c.Connect, f = YAHOO.lang, k = YAHOO.env.ua, m = k.ie, h = j.PLUGIN_TYPE, d = "ks-editor-image", r = "ks-editor-btn-ok", e = "ks-editor-btn-cancel", g = "ks-editor-image-tabs", p = "ks-editor-image-tab-content", n = "ks-editor-image-uploading", a = "ks-editor-dialog-actions", o = "ks-editor-image-no-tab", t = "ks-editor-image-tab-selected", s = {local:"<li rel=\"local\" class=\"" + t + "\">{tab_local}</li>", link:"<li rel=\"link\">{tab_link}</li>", album:"<li rel=\"album\">{tab_album}</li>"}, q = ["<form  name=\"upload_form\" action=\"uploadFile.action\" method=\"post\"  target=\"selfframe\" enctype=\"multipart/form-data\">", "<ul class=\"", g, " ks-clearfix\">", "</ul>", "<div class=\"", p, "\" rel=\"local\" style=\"display: none\">", "<label>{label_local}</label>", "<input type=\"file\" size=\"40\" name=\"imgFile\" unselectable=\"on\" />", "{local_extraCode}", "</div>", "<div class=\"", p, "\" rel=\"link\">", "<label>{label_link}</label>", "<input type=\"file\" name=\"imgUrl\" size=\"50\" />","<br>","{gao}", "<input name=\"gao\" type=\"text\" id=\"gao\" size=\"10\" /> px<br>","{kuan}","<input name=\"kuan\" type=\"text\" id=\"kuan\" size=\"10\" /> px","</div>", "<div class=\"", p, "\" rel=\"album\" style=\"display: none\">", "<label>{label_album}</label>", "<p style=\"width: 300px\">\u5c1a\u672a\u5b9e\u73b0...</p>", "</div>", "<div class=\"", n, "\" style=\"display: none\">", "<p style=\"width: 300px\">{uploading}</p>", "</div>", "<div class=\"", a, "\">", "<button name=\"ok\" onClick=\"upload_form.submit()\" class=\"", r, "\">{ok}</button>", "<span class=\"", e, "\">{cancel}</span>", "</div>", "</form>"].join(""), i = {tabs:["link"], upload:{actionUrl:"", filter:"png|gif|jpg|jpeg", filterMsg:"", enableXdr:false, connectionSwf:"http://a.tbcdn.cn/yui/2.8.0r4/build/connection/connection.swf", formatResponse:function (x) {
		var v = [];
		for (var w in x) {
			v.push(x[w]);
		}
	...
	j.addPlugin("image", {type:h.TOOLBAR_BUTTON, config:{}, dialog:null, form:null, range:null, currentTab:null, currentPanel:null, uploadingPanel:null, actionsBar:null, init:function () {
		var v = this.editor.config.pluginsConfig[this.name] || {};
		i.upload.filterMsg = this.lang.upload_filter;
		this.config = f.merge(i, v);
		this.config.upload = f.merge(i.upload, v.upload || {});
		this._renderUI();
		this._bindUI();
		this.actionsBar = b.getElementsByClassName(a, "div", this.dialog)[0];
		this.uploadingPanel = b.getElementsByClassName(n, "div", this.dialog)[0];
		this.config.upload.enableXdr && this._initXdrUpload();
	}, _renderUI:function () {
		var v = j.Menu.generateDropMenu(this.editor, this.domEl, [1, 0]), w = this.lang;
		w.local_extraCode = this.config.upload.extraCode;
		v.className += " " + d;
		v.innerHTML = q.replace(/\{([^}]+)\}/g, function (x, y) {
			return (y in w) ? w[y] : y;
		});
		this.dialog = v;
		this.form = v.getElementsByTagName("form")[0];
		if (m) {
			j.Dom.setItemUnselectable(v);
		}
		this._renderTabs();
	}, _renderTabs:function () {
		var v = this.lang, F = this, B = b.getElementsByClassName(g, "ul", this.dialog)[0], C = b.getElementsByClassName(p, "div", this.dialog);
		var E = this.config.tabs, z = "";
		for (var x = 0, w = E.length; x < w; x++) {
			z += s[E[x]];
		}
		B.innerHTML = z.replace(/\{([^}]+)\}/g, function (G, H) {
			return (H in v) ? v[H] : H;
		});
		...
		function y(I) {
			var H = 0, G = I.getAttribute("rel");
			for (var J = 0; J < A; J++) {
				if (D[J]) {
					b.removeClass(D[J], t);
				}
				C[J].style.display = "none";
				if (C[J].getAttribute("rel") == G) {
					H = J;
				}
			}
			if (k.ie === 6) {
				j.Menu._updateShimRegion(F.dialog);
			}
			b.addClass(I, t);
			C[H].style.display = "";
			F.currentTab = I.getAttribute("rel");
			F.currentPanel = C[H];
		}
	}, _bindUI:function () {
		var v = this;
		u.on(this.domEl, "click", function () {
			if (v.dialog.style.visibility === m ? "hidden" : "visible") {
				v._syncUI();
			}
		});
		u.on(this.dialog, "click", function (w) {
			var x = u.getTarget(w), y = v.currentTab;
			switch (x.className) {
			  case r:
				if (y === "local") {
					u.stopPropagation(w);
		...
		for (var w = 0, v = x.length; w < v; w++) {
			if (x[w] === "local") {
				l.transport(this.config.upload.connectionSwf);
				break;
			}
		}
		...
		if (y && z) {
			if (w.filter !== "*") {
				x = y.substring(y.lastIndexOf(".") + 1).toLowerCase();
			...
			if (k.ie === 6) {
				j.Menu._updateShimRegion(this.dialog);
			}
			l.setForm(A, true);
			l.asyncRequest("post", z, {upload:function (D) {
				try {
					var C = w.formatResponse(f.JSON.parse(D.responseText));
					if (C[0] == "0") {
						v._insertImage(C[1]);
						v._hideDialog();
					} else {
						v._onUploadError(C[1]);
					}
				}
				catch (B) {
					v._onUploadError(f.dump(B) + "\no = " + f.dump(D) + "\n[from upload catch code]");
				}
			}, xdr:w.enableXdr});
		} else {
			v._hideDialog();
		...
		if (v && b.isAncestor(v, this.dialog)) {
			j.Menu.hideActiveDropMenu(this.editor);
		}
		this.editor.contentWin.focus();
	}, _syncUI:function () {
		this.range = j.Range.saveRange(this.editor);
		this.form.reset();
		...
		if (window.getSelection) {
			var v = A.contentDoc.createElement("img");
			v.src = x;
			if (B) {
			...
			w.insertNode(v);
			if (k.webkit) {
				var z = A.contentWin.getSelection();
				z.addRange(w);
				z.collapseToEnd();
			} else {
				w.setStartAfter(v);
			...
			if (document.selection) {
				A.contentWin.focus();
				var gao=document.upload_form.gao.value;
				var kuan=document.upload_form.kuan.value;
					if ("text" in w) {
					w.select();
					
					if(gao!=null&&gao!="" || kuan !=null&&kuan!=""){
						var y = "<img src=\"" + x + "\" width=\""+kuan+"\" height=\""+gao+"\"";
						B && (y += " \"alt=\"" + B + "\"");
						y += ">";
					}else if(gao==null||gao=="" && kuan ==null||kuan==""){
						var y = "<img src=\"" + x + "\"";
						B && (y += " \"alt=\"" + B + "\"");
						y += ">";
					}
					...
					w.execCommand("insertImage", false, x);
				}
			}
		}
	}});
...
KISSY.Editor.add("plugins~indent", function (b) {
	var c = b.PLUGIN_TYPE, a = {type:c.TOOLBAR_BUTTON, exec:function () {
		this.editor.execCommand(this.name);
		this.editor.toolbar.updateState();
	}};
	b.addPlugin(["indent", "outdent"], a);
});
KISSY.Editor.add("plugins~justify", function (b) {
	var d = b.PLUGIN_TYPE, c = ["justifyLeft", "justifyCenter", "justifyRight"], a = {type:d.TOOLBAR_BUTTON, exec:function () {
		this.editor.execCommand(this.name);
...
KISSY.Editor.add("plugins~keystroke", function (c) {
	...
	c.addPlugin("keystroke", {type:e.FUNC, init:function () {
		var f = this.editor;
		if (b.ie && b.ie < 8) {
			a.on(f.contentDoc, "keydown", function (g) {
				if (g.keyCode == 9) {
...
KISSY.Editor.add("plugins~resize", function (d) {
	...
	d.addPlugin("resize", {type:f.STATUSBAR_ITEM, contentEl:null, currentHeight:0, init:function () {
		...
		var g = this.lang;
		this.domEl.innerHTML = c.replace(/\{([^}]+)\}/g, function (h, i) {
			return g[i] ? g[i] : i;
		});
	}, bindUI:function () {
		var h = this.domEl.getElementsByTagName("span"), j = h[0], g = h[1], i = this.contentEl;
...
KISSY.Editor.add("plugins~save", function (c) {
	var d = YAHOO.util, b = d.Event, e = c.PLUGIN_TYPE, a = {b:{tag:"strong"}, i:{tag:"em"}, u:{tag:"span", style:"text-decoration:underline"}, strike:{tag:"span", style:"text-decoration:line-through"}};
	c.addPlugin("save", {type:e.FUNC, init:function () {
		...
		f = f.replace(/<(\/?)([^>\s]+)([^>]*)>/g, function (i, k, h, g) {
			h = h.toLowerCase();
			var l = a[h], j = h;
			if (l && !g) {
				j = l.tag;
		...
		f = f.replace(/<(\w[^>]*) onmouseover="([^\"]*)"([^>]*)/gi, "<$1$3");
		f = f.replace(/<(\w[^>]*) onmouseout="([^\"]*)"([^>]*)/gi, "<$1$3");
		f = f.replace(/<H(\d)([^>]*)>/gi, "<h$1>");
		f = f.replace(/<(H\d)><FONT[^>]*>([\s\S]*?)<\/FONT><\/\1>/gi, "<$1>$2</$1>");
		f = f.replace(/<(H\d)><EM>([\s\S]*?)<\/EM><\/\1>/gi, "<$1>$2</$1>");
		f = f.replace(/<meta[^>]*>/ig, "");
		f = f.replace(/<link rel="\S+" href="file:[^>]*">/ig, "");
		f = f.replace(/<!--\[if gte mso [0-9]{1,2}\]>[\s\S]*?<!\[endif\]-->/ig, "");
		f = f.replace(/<style>[\s\S]*?mso[\s\S]*?<\/style>/ig, "");
		f = f.replace(/ lang=".+?"/ig, "");
...
KISSY.Editor.add("plugins~source", function (e) {
	...
	e.addPlugin("source", {type:g.TOOLBAR_BUTTON, init:function () {
		var h = this.editor;
		this.iframe = h.contentWin.frameElement;
		this.textarea = h.textarea;
		this.iframe.parentNode.appendChild(h.textarea);
		d.addClass(this.domEl, "ks-editor-toolbar-source-button");
	}, exec:function () {
		var h = this.editor, i = h.sourceMode;
		if (i) {
			h.contentDoc.body.innerHTML = this.textarea.value;
		} else {
			this.textarea.value = h.getContentDocData();
		}
		if (c.ie && c.ie < 8) {
			h.contentDoc.selection.empty();
		}
		this.textarea.style.display = i ? "none" : "";
		this.iframe.style.display = i ? "" : "none";
		h.sourceMode = !i;
...
KISSY.Editor.add("plugins~undo", function (a) {
	var b = a.PLUGIN_TYPE;
	a.addPlugin(["undo", "redo"], {type:b.TOOLBAR_BUTTON, exec:function () {
		this.editor.execCommand(this.name);
	}});
});
KISSY.Editor.add("plugins~wordcount", function (f) {
	var g = YAHOO.util, c = g.Dom, b = g.Event, e = YAHOO.lang, h = f.PLUGIN_TYPE, d = "ks-editor-wordcount-alarm", a = {total:50000, threshold:100};
	f.addPlugin("wordcount", {type:h.STATUSBAR_ITEM, total:Infinity, remain:Infinity, threshold:0, remainEl:null, init:function () {
		var j = e.merge(a, this.editor.config.pluginsConfig[this.name] || {});
		this.total = j.total;
		...
		this.domEl.innerHTML = this.lang.tmpl.replace("%remain%", "<em>" + this.total + "</em>");
		this.remainEl = this.domEl.getElementsByTagName("em")[0];
	}, bindUI:function () {
		var i = this.editor;
		b.on(i.textarea, "keyup", this.syncUI, this, true);
		b.on(i.contentDoc, "keyup", this.syncUI, this, true);
