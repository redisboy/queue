...
<%  
	List<QueueEvaluation> list = (List<QueueEvaluation>) request.getSession().getAttribute("list");
    response.reset();  
    response.setContentType("application/vnd.ms-excel;charset=UTF-8");  
    response.setHeader("Content-Disposition", "attachment;filename=" + new String("队列评价统计.xls".getBytes("gb2312"), "iso8859-1"));  
  
    OutputStream os = response.getOutputStream();  
  
    WritableWorkbook wwb = null;  
    wwb = Workbook.createWorkbook(os);//将 WritableWorkbook 写入到输出流  
    WritableSheet ws = wwb.createSheet("队列评价统计", 0);//创建第一个sheet  
    ...
    if(role.equals("0")){
    	...
    	ws.addCell(new Label(2, 0, "非常满意",headerFormat));
    	ws.addCell(new Label(3, 0, "满意",headerFormat));
    	ws.addCell(new Label(4, 0, "一般",headerFormat));
    	ws.addCell(new Label(5, 0, "不满意",headerFormat));
    	ws.addCell(new Label(6, 0, "次数(合计)",headerFormat));
    	ws.addCell(new Label(7, 0, "总分",headerFormat));
    	ws.addCell(new Label(8, 0, "平均分",headerFormat));
        for(int i=0;i<list.size();i++) {   
    	    ws.addCell(new Label(0, i+1, list.get(i).getName(),headerFormat));
    	    ws.addCell(new Label(1, i+1, list.get(i).getCode(),headerFormat));
    	    ws.addCell(new Label(2, i+1, list.get(i).getA1(),headerFormat));
    	    ws.addCell(new Label(3, i+1, list.get(i).getA2(),headerFormat));
    	    ws.addCell(new Label(4, i+1, list.get(i).getA3(),headerFormat));
    	    ws.addCell(new Label(5, i+1, list.get(i).getA4(),headerFormat));
    	    ws.addCell(new Label(6, i+1, list.get(i).getCountall(),headerFormat));
    	    ws.addCell(new Label(7, i+1, list.get(i).getSumcore(),headerFormat));
    	    ws.addCell(new Label(8, i+1, list.get(i).getAvgcore(),headerFormat));
    	}
    }
